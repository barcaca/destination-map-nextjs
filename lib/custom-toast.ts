 ...
 * ```
 */
export function customToast(data: ToastState): void {
  switch (data.status) {
    case 200:
    ...
    case 400:
      toast.error(
        data.message || 'Solicitação inválida. Verifique os dados fornecidos.'
      )
      break
    case 401:
    ...
    case 500:
      toast.error(
        data.message || 'Erro interno no servidor. Tente novamente mais tarde.'
      )
      break
    case 503:
      toast.warning(
